...
import { Send, User, CheckCheck, Loader2, MessageSquare, Briefcase, Check } from "lucide-react";
...
export default function EmployerMessagesPage() {
  ...
  useEffect(() => {
    ...
    // ایجاد یک کانال عمومی برای وضعیت حضور تمامی کاربران
    const presenceChannel = supabase.channel('global_presence');

    presenceChannel
      .on('presence', { event: 'sync' }, () => {
        const state = presenceChannel.presenceState();
        // استخراج آیدی تمام کاربرانی که الان تو این صفحه حضور دارن
        const onlineIds = Object.values(state).flatMap((clients: any) => clients.map((c: any) => c.user_id));
        setOnlineUsers(onlineIds);
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          // ثبت حضور خودم در شبکه
          ...
          <>
            ...
            <div className="flex items-center gap-4 p-4 border-b border-slate-100 bg-slate-50/50 relative z-10 shadow-sm">
              ...
              <div className="relative flex h-12 w-12 items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500">
                ...
                {/* 🔥 نقطه سبز هدر */}
                {onlineUsers.includes(activeChat.job_seeker_id) && (
                  <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full bg-green-500 border-2 border-white"></span>
                )}
              </div>
              <div>
                ...
                <p className="text-xs mt-0.5 font-medium flex items-center gap-1">
                  ...
                  <span className="text-slate-500">متقاضی موقعیت: {activeChat.job?.title}</span>
                </p>
              </div>
            </div>
